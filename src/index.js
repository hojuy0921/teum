...
export class TeumDatabase extends DurableObject {
  ...
  async adminMatch(req){if(!this.authorizedKey(req))return j({error:"관리자 인증이 필요합니다."},401);const b=await req.json().catch(function(){return {}});const wantedId=Number(b.wanted_id),providerId=Number(b.provider_id);if(!Number.isInteger(wantedId)||!Number.isInteger(providerId)||wantedId===providerId)return j({error:"매칭 대상이 올바르지 않습니다."},400);const wanted=this.sql.exec("SELECT * FROM submissions WHERE id=?",wantedId).toArray()[0];const provider=this.sql.exec("SELECT * FROM submissions WHERE id=?",providerId).toArray()[0];if(!wanted||!provider)return j({error:"매칭 대상을 찾을 수 없습니다."},404);if(wanted.type!=="구합니다"||provider.type==="구합니다")return j({error:"구합니다와 제공 글끼리만 매칭할 수 있습니다."},400);if(wanted.status!=="APPROVED"||provider.status!=="APPROVED")return j({error:"두 항목을 먼저 공개 승인해주세요."},400);this.sql.exec("INSERT OR IGNORE INTO matches(wanted_submission_id,provider_submission_id,status) VALUES(?,?,?)",wantedId,providerId,"MATCHED");const row=this.sql.exec("SELECT id,status FROM matches WHERE wanted_submission_id=? AND provider_submission_id=?",wantedId,providerId).toArray()[0];return j({ok:true,match_id:row.id,status:row.status})}
