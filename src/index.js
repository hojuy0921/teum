...
"<meta name='viewport' content='width=device-width,initial-scale=1'>",
"<title>TEUM 등록</title>",
"<style>body{margin:0;background:#f5f2ea;color:#171717;font-family:Arial,'Noto Sans KR',sans-serif}.wrap{max-width:700px;margin:auto;padding:32px 18px}.brand{font-size:26px;font-weight:900;margin-bottom:28px}.box{background:#fffdf8;border:1px solid #e5dfd4;border-radius:20px;padding:24px}.muted{color:#77716a;font-size:13px;line-height:1.65}.field{margin:13px 0}.field label{display:block;font-size:12px;font-weight:800;margin-bottom:6px}.field input,.field textarea,.field select{width:100%;box-sizing:border-box;border:1px solid #ddd5ca;border-radius:10px;padding:12px;background:#fff;font:inherit}.field textarea{min-height:120px;resize:vertical}.row{display:grid;grid-template-columns:1fr 1fr;gap:10px}.actions{display:flex;justify-content:flex-end;gap:8px;margin-top:18px}.btn{border:1px solid #ddd5ca;background:#fff;border-radius:10px;padding:12px 15px;font-weight:800;cursor:pointer}.dark{background:#171717;color:#fff;border-color:#171717}.ok{text-align:center;padding:30px}@media(max-width:600px){.row{grid-template-columns:1fr}}</style>",
"</head><body><main class='wrap'>",
"<div class='brand'>틈 <span style='font-size:10px;color:#888'>TEUM</span></div>",
"<div class='box' id='applyBox'>",
"<h1 style='letter-spacing:-.05em'>필요한 것, 팔고 싶은 것,<br>할 수 있는 것을 남겨주세요.</h1>",
"<p class='muted'>연락처는 공개되지 않습니다. 운영자가 확인한 뒤 조건이 맞는 사람을 연결합니다.</p>",
"<div class='field'><label>종류</label><select id='applyType'><option>구합니다</option><option>팝니다</option><option>레슨</option><option>서비스</option><option>수제품</option></select></div>",
"<div class='field'><label>닉네임</label><input id='applyNickname' maxlength='40'></div>",
"<div class='field'><label>무엇이 필요한가요 / 무엇을 제공하나요?</label><input id='applyTitle' maxlength='120'></div>",
"<div class='row'><div class='field'><label>지역</label><input id='applyCity' maxlength='80' placeholder='김해 / 부산 / 온라인'></div><div class='field'><label>가격 / 예산</label><input id='applyPrice' maxlength='50' placeholder='50,000원 / 협의'></div></div>",
"<div class='field'><label>가능한 시간</label><input id='applyTime' maxlength='100' placeholder='토요일 오후 / 평일 저녁 / 언제든'></div>",
"<div class='field'><label>상세 내용</label><textarea id='applyDescription' maxlength='4000'></textarea></div>",
"<div class='field'><label>연락받을 방법</label><input id='applyContact' maxlength='300' placeholder='오픈채팅 링크 / 이메일'></div>",
"<div class='field'><label>연결 동의</label><select id='applyConsent'><option value='yes'>네, 조건이 맞는 사람과 연결해주세요.</option><option value='no'>아니요.</option></select></div>",
"<div class='actions'><button class='btn' id='backBtn'>돌아가기</button><button class='btn dark' id='submitBtn'>등록하기</button></div>",
"</div>",
"<script>",
"document.getElementById('backBtn').onclick=function(){location.href='/'};",
"document.getElementById('submitBtn').onclick=async function(){",
"var b={type:document.getElementById('applyType').value,nickname:document.getElementById('applyNickname').value.trim(),title:document.getElementById('applyTitle').value.trim(),city:document.getElementById('applyCity').value.trim(),price:document.getElementById('applyPrice').value.trim(),time:document.getElementById('applyTime').value.trim(),description:document.getElementById('applyDescription').value.trim(),contact:document.getElementById('applyContact').value.trim(),consent:document.getElementById('applyConsent').value};",
"if(!b.nickname||!b.title||!b.description||!b.contact){alert('필수 항목을 입력해주세요.');return}",
"if(b.consent!=='yes'){alert('연결 동의를 확인해주세요.');return}",
"try{var r=await fetch('/api/submissions',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)});var d=await r.json();if(!r.ok)throw Error(d.error||'등록에 실패했습니다.');document.getElementById('applyBox').innerHTML='<div class=ok><b>등록 완료</b><p class=muted>TEUM 운영자가 확인하고 조건이 맞는 사람을 찾아 연결해드릴게요.</p><button class=\'btn dark\' id=\'doneBtn\'>TEUM으로 돌아가기</button></div>';document.getElementById('doneBtn').onclick=function(){location.href='/'};}catch(e){alert(e.message)}};",
"</script></main></body></html>"
].join("");

const ADMIN = `<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>TEUM 관리자</title><style>body{margin:0;background:#f5f2ea;color:#171717;font-family:Arial,"Noto Sans KR",sans-serif}.wrap{max-width:1100px;margin:auto;padding:25px 18px}.head{display:flex;justify-content:space-between;align-items:end}.box,.entry{background:#fffdf8;border:1px solid #e5dfd4;border-radius:16px;padding:17px;margin:13px 0}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.entry{background:#fff}.muted{font-size:12px;color:#77716a;line-height:1.6}.row{display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}.row input{flex:1;min-width:220px;border:1px solid #ddd5ca;border-radius:9px;padding:10px}.btn{border:1px solid #ddd5ca;background:#fff;border-radius:9px;padding:9px 12px;font-weight:800;cursor:pointer}.ok{background:#d9ff51;border-color:#b9df35}.bad{background:#fff1f0;color:#9a2c2c}.hide{display:none}.match{margin-top:11px;padding-top:11px;border-top:1px solid #eee7dc}@media(max-width:700px){.grid{grid-template-columns:1fr}.head{display:block}}</style></head><body><main class="wrap"><div class="head"><div><h1>TEUM 관리자</h1><div class="muted">접수 → 검토 → 공개 → 매칭</div></div><a href="/">사이트</a></div><div id="gate" class="box"><b>관리자 키</b><p class="muted">Cloudflare Secret <code>TEUM_ADMIN_KEY</code>에 설정한 값을 입력하세요.</p><div class="row"><input id="key" type="password" placeholder="관리자 키"><button class="btn" onclick="connect()">접속</button></div></div><div id="app" class="hide"><div class="box"><b id="stats">불러오는 중...</b><button class="btn" style="float:right" onclick="load()">새로고침</button></div><div id="list" class="grid"></div></div><script>var KEY="";function esc(x){return String(x==null?"":x).replace(/[&<>"]/g,function(m){return {"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[m]})}async function api(u,o){o=o||{};var r=await fetch(u,Object.assign({},o,{headers:Object.assign({"Authorization":"Bearer "+KEY,"Content-Type":"application/json"},o.headers||{})})),d=await r.json().catch(function(){return {}});if(!r.ok)throw Error(d.error||"실패");return d}async function connect(){KEY=document.getElementById("key").value.trim();try{await load();sessionStorage.setItem("teum_admin",KEY);gate.classList.add("hide");app.classList.remove("hide")}catch(e){alert(e.message)}}async function load(){KEY=KEY||sessionStorage.getItem("teum_admin")||"";var d=await api("/api/admin/submissions");stats.textContent="전체 "+d.stats.total+" · 대기 "+d.stats.pending;list.innerHTML=d.entries.map(function(e){var m=(e.matches||[]).map(function(x){return '<div class="muted">→ '+esc(x.title)+' · '+esc(x.nickname)+' · 점수 '+esc(x.score)+'</div>'}).join("");return '<article class="entry"><b>'+esc(e.type)+'</b><h3>'+esc(e.title)+'</h3><div class="muted">'+esc(e.nickname)+' · '+esc(e.city||"지역 미정")+' · '+esc(e.created_at)+'</div><p>'+esc(e.description)+'</p><div class="muted">가격/예산: '+esc(e.price||"협의")+' · 시간: '+esc(e.time||"미정")+'<br>연락: <b>'+esc(e.contact)+'</b></div><div class="row"><button class="btn ok" onclick="statusDo('+e.id+',\\'APPROVED\\')">공개</button><button class="btn" onclick="statusDo('+e.id+',\\'HIDDEN\\')">숨기기</button><button class="btn bad" onclick="statusDo('+e.id+',\\'REJECTED\\')">거절</button></div>'+(m?'<div class="match"><b>매칭 후보</b>'+m+'</div>':"")+'</article>'}).join("")||'<div class="box">접수된 항목이 없습니다.</div>'}async function statusDo(id,s){try{await api("/api/admin/submissions/"+id+"/status",{method:"POST",body:JSON.stringify({status:s})});await load()}catch(e){alert(e.message)}}if(sessionStorage.getItem("teum_admin")){KEY=sessionStorage.getItem("teum_admin");load().then(function(){gate.classList.add("hide");app.classList.remove("hide")}).catch(function(){sessionStorage.removeItem("teum_admin")})}</script></main></body></html>`;
...
export class TeumDatabase extends DurableObject {
  ...
  async adminStatus(req){if(!this.authorized(req))return j({error:"관리자 인증이 필요합니다."},401);const id=Number(new URL(req.url).pathname.split("/")[4]);const b=await req.json();if(!["PENDING","APPROVED","HIDDEN","REJECTED"].includes(b.status))return j({error:"상태값이 올바르지 않습니다."},400);this.sql.exec("UPDATE submissions SET status=? WHERE id=?",b.status,id);return j({ok:true})}
}
