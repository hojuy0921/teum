  ...
  async fetch(request, env) {
    ...
    if (url.pathname.startsWith("/api/")) {
      ...
      if (url.pathname === "/api/admin/submissions" || (url.pathname.indexOf("/api/admin/submissions/") === 0 && url.pathname.endsWith("/status"))) {
        ...
        headers.set("x-teum-admin-secret", String(env.TEUM_ADMIN_KEY || ""));
...
export class TeumDatabase extends DurableObject {
  ...
  authorizedKey(got,expectedValue){const expected=String(expectedValue||this.env.TEUM_ADMIN_KEY||"").trim().normalize("NFKC");const actual=String(got||"").trim().normalize("NFKC");return !!expected&&!!actual&&actual===expected}
  ...
  async adminSubmissions(req){let key="";if(req.method==="POST"){const b=await req.json().catch(function(){return {}});key=String(b.key||"")}else{key=String(req.headers.get("Authorization")||"").replace(/^Bearer /,"")}const expected=String(req.headers.get("x-teum-admin-secret")||this.env.TEUM_ADMIN_KEY||"").trim();if(!expected)return j({error:"Cloudflare에 TEUM_ADMIN_KEY Secret이 설정되지 않았습니다."},500);if(!this.authorizedKey(key,expected))return j({error:"관리자 키가 올바르지 않습니다."},401);const rows=this.sql.exec("SELECT * FROM submissions ORDER BY id DESC LIMIT 300").toArray();return j({entries:rows.map(x=>Object.assign({},x,{matches:this.submissionMatches(x.id)})),stats:{total:rows.length,pending:rows.filter(x=>x.status==="PENDING").length}})}
  ...
  async adminStatus(req){const b=await req.json().catch(function(){return {}});const expected=String(req.headers.get("x-teum-admin-secret")||this.env.TEUM_ADMIN_KEY||"").trim();if(!expected)return j({error:"Cloudflare에 TEUM_ADMIN_KEY Secret이 설정되지 않았습니다."},500);if(!this.authorizedKey(b.key,expected))return j({error:"관리자 키가 올바르지 않습니다."},401);const id=Number(new URL(req.url).pathname.split("/")[4]);if(!["PENDING","APPROVED","HIDDEN","REJECTED"].includes(b.status))return j({error:"상태값이 올바르지 않습니다."},400);this.sql.exec("UPDATE submissions SET status=? WHERE id=?",b.status,id);return j({ok:true})}
}
