...
async function replyMsg(post_id,receiver_id,inputId){try{var el=$("#"+inputId),body=(el&&el.value||"").trim();if(!body){toast("답장 내용을 입력하세요.");return}await api("/api/messages",{method:"POST",body:JSON.stringify({post_id:post_id,receiver_id:receiver_id,body:body})});await messages()}catch(e){toast(e.message)}}
