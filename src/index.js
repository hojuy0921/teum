...
export class TeumDatabase extends DurableObject {
  ...
  async adminSubmissions(req){let key="";if(req.method==="POST"){const b=await req.json().catch(function(){return {}});key=String(b.key||"")}else{key=String(req.headers.get("Authorization")||"").replace(/^Bearer /,"")}if(!String(this.env.TEUM_ADMIN_KEY||"").trim())return j({error:"Cloudflare에 TEUM_ADMIN_KEY Secret이 설정되지 않았습니다."},500);if(!this.authorizedKey(key))return j({error:"관리자 키가 올바르지 않습니다. Cloudflare의 TEUM_ADMIN_KEY 값과 입력값이 정확히 같은지 확인하세요."},401);const rows=this.sql.exec("SELECT * FROM submissions ORDER BY id DESC LIMIT 300").toArray();return j({entries:rows.map(x=>Object.assign({},x,{matches:this.submissionMatches(x.id)})),stats:{total:rows.length,pending:rows.filter(x=>x.status==="PENDING").length}})}
