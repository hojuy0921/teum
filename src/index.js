...
const APPLY="<!doctype html><html lang=\"ko\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>TEUM 등록</title><style>body{margin:0;background:#f5f2ea;color:#171717;font-family:Arial,\"Noto Sans KR\",sans-serif}.wrap{max-width:700px;margin:auto;padding:32px 18px}.brand{font-size:26px;font-weight:900;margin-bottom:28px}.box{background:#fffdf8;border:1px solid #e5dfd4;border-radius:20px;padding:24px}.muted{color:#77716a;font-size:13px;line-height:1.65}.field{margin:13px 0}.field label{display:block;font-size:12px;font-weight:800;color:#706960;margin-bottom:6px}.field input,.field textarea,.field select{width:100%;box-sizing:border-box;border:1px solid #ddd5ca;border-radius:10px;padding:12px;background:#fff;font:inherit}.field textarea{min-height:120px;resize:vertical}.row{display:grid;grid-template-columns:1fr 1fr;gap:10px}.actions{display:flex;justify-content:flex-end;gap:8px;margin-top:18px}.btn{border:1px solid #ddd5ca;background:#fff;border-radius:10px;padding:12px 15px;font-weight:800;cursor:pointer}.dark{background:#171717;color:#fff;border-color:#171717}.ok{text-align:center;padding:30px}.ok b{font-size:25px}@media(max-width:600px){.row{grid-template-columns:1fr}}</style></head><body><main class=\"wrap\"><div class=\"brand\">틈 <span style=\"font-size:10px;color:#888\">TEUM</span></div><div class=\"box\" id=\"box\"><h1 style=\"letter-spacing:-.05em\">필요한 것, 팔고 싶은 것,<br>할 수 있는 것을 남겨주세요.</h1><p class=\"muted\">연락처는 공개되지 않습니다. 운영자가 확인한 뒤 조건이 맞는 사람을 연결합니다.</p><div class=\"field\"><label>종류</label><select id=\"type\"><option>구합니다</option><option>팝니다</option><option>레슨</option><option>서비스</option><option>수제품</option></select></div><div class=\"field\"><label>닉네임</label><input id=\"nickname\" maxlength=\"40\" placeholder=\"예: 민수\"></div><div class=\"field\"><label>무엇이 필요한가요 / 무엇을 제공하나요?</label><input id=\"title\" maxlength=\"120\" placeholder=\"예: 5만원 이하 기타 / 기타 초보 레슨\"></div><div class=\"row\"><div class=\"field\"><label>지역</label><input id=\"city\" maxlength=\"80\" placeholder=\"김해 / 부산 / 온라인\"></div><div class=\"field\"><label>가격 / 예산</label><input id=\"price\" maxlength=\"50\" placeholder=\"50,000원 / 협의\"></div></div><div class=\"field\"><label>가능한 시간</label><input id=\"time\" maxlength=\"100\" placeholder=\"토요일 오후 / 평일 저녁 / 언제든\"></div><div class=\"field\"><label>상세 내용</label><textarea id=\"description\" maxlength=\"4000\" placeholder=\"상태, 경험, 원하는 조건, 장소 등을 적어주세요.\"></textarea></div><div class=\"field\"><label>연락받을 방법</label><input id=\"contact\" maxlength=\"300\" placeholder=\"오픈채팅 링크 / 이메일\"></div><div class=\"field\"><label>연결 동의</label><select id=\"consent\"><option value=\"yes\">네, 조건이 맞는 사람과 연결해주세요.</option><option value=\"no\">아니요.</option></select></div><div class=\"actions\"><button class=\"btn\" onclick=\"location.href='/'\">돌아가기</button><button class=\"btn dark\" onclick=\"submitIt()\">등록하기</button></div></div><script>async function submitIt(){const b={type:type.value,nickname:nickname.value,title:title.value,city:city.value,price:price.value,time:time.value,description:description.value,contact:contact.value,consent:consent.value};if(!b.nickname||!b.title||!b.description||!b.contact){alert(\"필수 항목을 입력해주세요.\");return}if(b.consent!==\"yes\"){alert(\"연결 동의를 확인해주세요.\");return}try{const r=await fetch(\"/api/submissions\",{method:\"POST\",headers:{\"Content-Type\":\"application/json\"},body:JSON.stringify(b)}),d=await r.json();if(!r.ok)throw Error(d.error||\"등록에 실패했습니다.\");box.innerHTML='<div class=\"ok\"><b>등록 완료</b><p class=\"muted\">TEUM 운영자가 확인하고 조건이 맞는 사람을 찾아 연결해드릴게요.</p><button class=\"btn dark\" onclick=\"location.href=\\'/\\'\">TEUM으로 돌아가기</button></div>'}catch(e){alert(e.message)}}</script></main></body></html>";
const ADMIN="<!doctype html><html lang=\"ko\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>TEUM 관리자</title><style>body{margin:0;background:#f5f2ea;color:#171717;font-family:Arial,\"Noto Sans KR\",sans-serif}.wrap{max-width:1100px;margin:auto;padding:25px 18px}.head{display:flex;justify-content:space-between;align-items:end}.box,.entry{background:#fffdf8;border:1px solid #e5dfd4;border-radius:16px;padding:17px;margin:13px 0}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.entry{background:#fff}.muted{font-size:12px;color:#77716a;line-height:1.6}.row{display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}.row input{flex:1;min-width:220px;border:1px solid #ddd5ca;border-radius:9px;padding:10px}.btn{border:1px solid #ddd5ca;background:#fff;border-radius:9px;padding:9px 12px;font-weight:800;cursor:pointer}.ok{background:#d9ff51;border-color:#b9df35}.bad{background:#fff1f0;color:#9a2c2c}.hide{display:none}.match{margin-top:11px;padding-top:11px;border-top:1px solid #eee7dc}@media(max-width:700px){.grid{grid-template-columns:1fr}.head{display:block}}</style></head><body><main class=\"wrap\"><div class=\"head\"><div><h1>TEUM 관리자</h1><div class=\"muted\">접수 → 검토 → 공개 → 매칭</div></div><a href=\"/\">사이트</a></div><div id=\"gate\" class=\"box\"><b>관리자 키</b><p class=\"muted\">Cloudflare Secret <code>TEUM_ADMIN_KEY</code>에 설정한 값을 입력하세요.</p><div class=\"row\"><input id=\"key\" type=\"password\" placeholder=\"관리자 키\"><button class=\"btn\" onclick=\"connect()\">접속</button></div></div><div id=\"app\" class=\"hide\"><div class=\"box\"><b id=\"stats\">불러오는 중...</b><button class=\"btn\" style=\"float:right\" onclick=\"load()\">새로고침</button></div><div id=\"list\" class=\"grid\"></div></div><script>let KEY=\"\";function esc(x){return String(x??\"\").replace(/[&<>\"]/g,m=>({\"&\":\"&amp;\",\"<\":\"&lt;\",\">\":\"&gt;\",'\"':\"&quot;\"}[m]))}async function api(u,o={}){const r=await fetch(u,{...o,headers:{\"Authorization\":\"Bearer \"+KEY,...(o.headers||{})}}),d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||\"실패\");return d}async function connect(){KEY=document.getElementById(\"key\").value.trim();try{await load();sessionStorage.setItem(\"teum_admin\",KEY);gate.classList.add(\"hide\");app.classList.remove(\"hide\")}catch(e){alert(e.message)}}async function load(){KEY=KEY||sessionStorage.getItem(\"teum_admin\")||\"\";const d=await api(\"/api/admin/submissions\");stats.textContent=\"전체 \"+d.stats.total+\" · 대기 \"+d.stats.pending;list.innerHTML=d.entries.map(e=>'<article class=\"entry\"><span class=\"tag\">'+esc(e.type)+'</span><h3>'+esc(e.title)+'</h3><div class=\"muted\">'+esc(e.nickname)+' · '+esc(e.city||\"지역 미정\")+' · '+esc(e.created_at)+'</div><p>'+esc(e.description)+'</p><div class=\"muted\">가격/예산: '+esc(e.price||\"협의\")+' · 시간: '+esc(e.time||\"미정\")+'<br>연락: <b>'+esc(e.contact)+'</b></div><div class=\"row\"><button class=\"btn ok\" onclick=\"status('+e.id+',\\'APPROVED\\')\">공개</button><button class=\"btn\" onclick=\"status('+e.id+',\\'HIDDEN\\')\">숨기기</button><button class=\"btn bad\" onclick=\"status('+e.id+',\\'REJECTED\\')\">거절</button></div>'+(e.matches&&e.matches.length?'<div class=\"match\"><b>매칭 후보</b>'+e.matches.map(m=>'<div class=\"muted\">→ '+esc(m.title)+' · '+esc(m.nickname)+' · 점수 '+esc(m.score)+'</div>').join(\"\")+'</div>':\"\")+'</article>').join(\"\")||'<div class=\"box\">접수된 항목이 없습니다.</div>'}async function status(id,s){try{await api(\"/api/admin/submissions/\"+id+\"/status\",{method:\"POST\",headers:{\"Content-Type\":\"application/json\"},body:JSON.stringify({status:s})});await load()}catch(e){alert(e.message)}}if(sessionStorage.getItem(\"teum_admin\")){KEY=sessionStorage.getItem(\"teum_admin\");load().then(()=>{gate.classList.add(\"hide\");app.classList.remove(\"hide\")}).catch(()=>sessionStorage.removeItem(\"teum_admin\"))}</script></main></body></html>";
