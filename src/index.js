...
async function replyMsg(post_id,receiver_id,inputId){try{var el=$("#"+inputId),body=(el&&el.value||"").trim();if(!body){toast("답장 내용을 입력하세요.");return}await api("/api/messages",{method:"POST",body:JSON.stringify({post_id:0,receiver_id:receiver_id,body:body})});await messages()}catch(e){toast(e.message)}}
async function messages(){if(!me){auth();return}try{var d=await api("/api/messages"),list=d.messages||[],groups={};list.forEach(function(m){var other=m.sender_id===me.id?m.receiver_id:m.sender_id,key=String(m.post_id)+"_"+String(other);if(!groups[key])groups[key]={post_id:m.post_id,other_id:other,title:m.title||"대화",items:[]};groups[key].items.push(m)});var arr=Object.keys(groups).map(function(k){return groups[k]}).sort(function(a,b){return (b.items[b.items.length-1].id||0)-(a.items[a.items.length-1].id||0)});var h='<button class="close" onclick="closeModal()">×</button><h2>메시지</h2>';h+=arr.length?arr.map(function(g,i){var inputId="reply_"+i;var items=g.items.map(function(m){var mine=m.sender_id===me.id;return '<div style="display:flex;justify-content:'+(mine?'flex-end':'flex-start')+';margin:8px 0"><div style="max-width:78%;background:'+(mine?'#171717':'#eee9df')+';color:'+(mine?'#fff':'#171717')+';border-radius:13px;padding:10px 12px"><div style="font-size:11px;opacity:.65;margin-bottom:4px">'+(mine?'나':esc(m.sender_name))+'</div><div style="white-space:pre-wrap;line-height:1.55">'+esc(m.body)+'</div></div></div>'}).join("");return '<div style="padding:14px 0;border-bottom:1px solid #e7e1d7"><b>'+esc(g.title)+'</b>'+items+'<div class="field" style="margin-bottom:6px"><textarea id="'+inputId+'" placeholder="답장 보내기"></textarea></div><div class="actions" style="margin-top:6px"><button class="btn dark" onclick="replyMsg('+g.post_id+','+g.other_id+',\''+inputId+'\')">답장</button></div></div>'}).join(""):'<div class="empty">메시지가 없습니다.</div>';modal(h)}catch(e){toast(e.message)}}
...
export class TeumDatabase extends DurableObject {
  ...
  async sendMessage(req){const u=this.me(req);if(!u)return j({error:"로그인이 필요합니다."},401);const b=await req.json(),postId=Number(b.post_id||0),receiverId=Number(b.receiver_id),p=postId>0?this.sql.exec("SELECT * FROM posts WHERE id=?",postId).toArray()[0]:null;if(!Number.isInteger(receiverId)||receiverId<1||receiverId===u.id)return j({error:"메시지를 보낼 수 없습니다."},400);const existing=this.sql.exec("SELECT id,post_id FROM messages WHERE (sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?) ORDER BY id DESC LIMIT 1",u.id,receiverId,receiverId,u.id).toArray()[0];if(postId>0){if(!p)return j({error:"게시글을 찾을 수 없습니다."},404);if(p.user_id!==receiverId&&!existing)return j({error:"이 대화에 답장할 수 없습니다."},403)}else if(!existing){return j({error:"대화를 찾을 수 없습니다."},404)}const body=String(b.body||"").trim();if(!body)return j({error:"메시지를 입력하세요."},400);const savePostId=postId>0?postId:Number(existing.post_id||0);this.sql.exec("INSERT INTO messages(post_id,sender_id,receiver_id,body) VALUES(?,?,?,?,?)",savePostId,u.id,receiverId,body.slice(0,2000));return j({ok:true},201)}
  async replyMessage(req){const u=this.me(req);if(!u)return j({error:"로그인이 필요합니다."},401);const b=await req.json();const receiverId=Number(b.receiver_id);const body=String(b.body||"").trim();if(!Number.isInteger(receiverId)||receiverId<1||receiverId===u.id)return j({error:"상대방을 찾을 수 없습니다."},400);if(!body)return j({error:"답장 내용을 입력하세요."},400);const thread=this.sql.exec("SELECT post_id FROM messages WHERE (sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?) ORDER BY id DESC LIMIT 1",u.id,receiverId,receiverId,u.id).toArray()[0];if(!thread)return j({error:"대화를 찾을 수 없습니다."},404);const postId=Number(thread.post_id)||0;this.sql.exec("INSERT INTO messages(post_id,sender_id,receiver_id,body) VALUES(?,?,?,?,?)",postId,u.id,receiverId,body.slice(0,2000));return j({ok:true},201)}
  messages(req){const u=this.me(req);if(!u)return j({error:"로그인이 필요합니다."},401);return j({messages:this.sql.exec("SELECT m.*,s.name sender_name,COALESCE(p.title,'TEUM 매칭 대화') title FROM messages m JOIN users s ON s.id=m.sender_id LEFT JOIN posts p ON p.id=m.post_id WHERE m.sender_id=? OR m.receiver_id=? ORDER BY m.id ASC LIMIT 200",u.id,u.id).toArray()})}
