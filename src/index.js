...
export class TeumDatabase extends DurableObject {
  ...
  syncApprovedSubmissions(){const operator=this.sql.exec("SELECT id FROM users WHERE username=?","__teum_operator__").toArray()[0];if(!operator)return;const rows=this.sql.exec("SELECT * FROM submissions WHERE status='APPROVED' ORDER BY id ASC LIMIT 300").toArray();rows.forEach(function(current){const exists=this.sql.exec("SELECT id FROM posts WHERE source_submission_id=?",current.id).toArray()[0];const rawPrice=String(current.price||"").replace(/[^0-9]/g,"");const price=rawPrice?Math.floor(Number(rawPrice)):null;const desc=String(current.description||"")+(current.time?"\n\n가능한 시간: "+String(current.time):"");if(exists){this.sql.exec("UPDATE posts SET type=?,title=?,description=?,price=?,city=?,tags=?,status='OPEN' WHERE source_submission_id=?",current.type,current.title,desc,price,current.city||"",current.time?("운영접수,"+current.time):"운영접수",current.id)}else{this.sql.exec("INSERT INTO posts(user_id,type,title,description,price,city,tags,image,status,source_submission_id) VALUES(?,?,?,?,?,?,?,?,?,?)",operator.id,current.type,current.title,desc,price,current.city||"",current.time?("운영접수,"+current.time):"운영접수","","OPEN",current.id)}}.bind(this))}
