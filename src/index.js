...
export class TeumDatabase extends DurableObject {
  ...
  async sendMessage(req){const u=this.me(req);if(!u)return j({error:"로그인이 필요합니다."},401);const b=await req.json(),postId=Number(b.post_id),receiverId=Number(b.receiver_id),p=this.sql.exec("SELECT * FROM posts WHERE id=?",postId).toArray()[0];if(!p||!Number.isInteger(receiverId)||receiverId<1||receiverId===u.id)return j({error:"메시지를 보낼 수 없습니다."},400);const existing=this.sql.exec("SELECT id FROM messages WHERE post_id=? AND ((sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?)) LIMIT 1",postId,u.id,receiverId,receiverId,u.id).toArray()[0];if(p.user_id!==receiverId&&!existing)return j({error:"이 대화에 답장할 수 없습니다."},403);const body=String(b.body||"").trim();if(!body)return j({error:"메시지를 입력하세요."},400);this.sql.exec("INSERT INTO messages(post_id,sender_id,receiver_id,body) VALUES(?,?,?,?,?)",p.id,u.id,receiverId,body.slice(0,2000));return j({ok:true},201)}
