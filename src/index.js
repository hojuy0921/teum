...
export class TeumDatabase extends DurableObject {
  ...
  authorizedKey(got){const expected=String(this.env.TEUM_ADMIN_KEY||"").trim().normalize("NFKC");const actual=String(got||"").trim().normalize("NFKC");return !!expected&&!!actual&&actual===expected}
  ...
  async adminSubmissions(req){let key="";if(req.method==="POST"){const b=await req.json().catch(function(){return {}});key=String(b.key||"")}else{key=String(req.headers.get("Authorization")||"").replace(/^Bearer /,"")}if(!this.authorizedKey(key))return j({error:"관리자 키가 올바르지 않습니다."},401);const rows=this.sql.exec("SELECT * FROM submissions ORDER BY id DESC LIMIT 300").toArray();return j({entries:rows.map(x=>Object.assign({},x,{matches:this.submissionMatches(x.id)})),stats:{total:rows.length,pending:rows.filter(x=>x.status==="PENDING").length}})}
  ...
  async adminStatus(req){const b=await req.json().catch(function(){return {}});if(!this.authorizedKey(b.key))return j({error:"관리자 키가 올바르지 않습니다."},401);const id=Number(new URL(req.url).pathname.split("/")[4]);if(!["PENDING","APPROVED","HIDDEN","REJECTED"].includes(b.status))return j({error:"상태값이 올바르지 않습니다."},400);this.sql.exec("UPDATE submissions SET status=? WHERE id=?",b.status,id);return j({ok:true})}
}
